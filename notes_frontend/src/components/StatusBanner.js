...
// PUBLIC_INTERFACE
export function StatusBanner({ kind = "info", title, message, actionLabel, onAction }) {
  ...
  return (
    <div className={`banner banner-${kind}`} role={kind === "error" ? "alert" : "status"}>
      ...
      {actionLabel && onAction ? (
        <button className="bannerAction" type="button" onClick={onAction} aria-label={actionLabel}>
          {actionLabel}
        </button>
      ) : null}
    </div>
